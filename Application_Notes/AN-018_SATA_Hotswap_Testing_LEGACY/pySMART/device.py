...
class Device(object):
    ...
    def __init__(self, name, interface=None):
        ...
        assert interface is None or interface.lower() in [
            'ata', 'csmi', 'sas', 'sat', 'sata', 'scsi', 'usbjmicron']
    ...
    def update(self):
        ...
        print(smartctl_type[self.interface])
        try:
            cmd = Popen('smartctl -d {0} -a /dev/{1}'.format(smartctl_type[self.interface], self.name), shell=True,stdout=PIPE, stderr=PIPE)
        except:
            smartctl_type["nvme"] = "ata"
            cmd = Popen('smartctl -d {0} -a /dev/{1}'.format(smartctl_type[self.interface], self.name), shell=True,stdout=PIPE, stderr=PIPE)
